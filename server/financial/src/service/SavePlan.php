...
class SavePlan extends BaseService{
    ...
    public function exec(){
        ...
        $detail = $commonLogic->uuidPriv($plan_uuid, $sessionToken, $planDb);
        if(!in_array($detail['plan_status'], [$planDb::PLAN_STATUS_SAVED, $planDb::PLAN_STATUS_REJECT, $planDb::PLAN_STATUS_CHECK_REJECT])){
            throw new \Exception('审核状态的理财计划不能编辑', ErrMsg::RET_CODE_SERVICE_FAIL);
        }

        try{
            $planUuid = $planDb->savePlanAndCash($planData, $cashData, $plan_uuid);
    ...
    protected function planParamCheck($sessionData){
        $data = [
            ...
            'rate_over_date' => $this->m_request['rate_over_date'] ?? null,
        ...
        if ($data['term_type'] == 2 && strtotime($data['rate_start_date']) >= strtotime($data['rate_over_date'])) {
            throw new \Exception('起息日必须小于投资产品到期日', ErrMsg::RET_CODE_GENVERIFY_TYPE_ERROR);
        }
