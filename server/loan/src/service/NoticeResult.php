	...
	public function exec()
	{
		...
		try
		{
			...
			if($this->m_request['status']==3){
				//成功
				$date = date('Y-m-d');
				$tran->params = ['uuid'=>$info[0]['uuid'],
					'loan_status'=>LoanTransfer::LOAN_STATUS_PAID,
					'transfer_status'=>LoanTransfer::TRANSFER_STATUS_ARCHIVE,
				...
				$ret = JmfUtil::call_Jmf_consumer('com.jyblife.logic.bg.flow.Approve',
						['flow_code'=>'loan_transfer_pay_type_2_code','instance_id'=>$info[0]['uuid'],'approve_type'=>'1']);
				...
				$tran->saveOrUpdate();
				$ret = JmfUtil::call_Jmf_consumer('com.jyblife.logic.bg.flow.Approve',
						['flow_code'=>'loan_transfer_pay_type_2_code','instance_id'=>$info[0]['uuid'],'approve_type'=>'1','info'=>$this->m_request['err_msg']]);
